...
function updateContinueBtn() {
  ...
  if (btn) {
    var hasBarber =
      document.querySelector(".barber-select-card.selected") !== null ||
      document.getElementById("step2-barber-id") !== null;
    var hasService =
      document.querySelector(".service-select-card.selected") !== null;
...
function loadAvailableSlots() {
  ...
  slotsGrid.innerHTML =
    '<p style="color:#888;font-size:13px;padding:8px 0;">Cargando horarios...</p>';
  ...
  fetch("/api/available-slots?barber_id=" + barberId + "&date=" + dateVal)
    ...
    .then(function (data) {
      ...
      if (!data.slots || data.slots.length === 0) {
        slotsGrid.innerHTML =
          '<p style="color:#888;font-size:13px;">No hay horarios disponibles para esta fecha.</p>';
        return;
      }
    ...
    .catch(function (err) {
      slotsGrid.innerHTML =
        '<p style="color:#E8302A;font-size:13px;">Error al cargar horarios. Intentá de nuevo.</p>';
    });
}
...
function openServiceModal(mode, data) {
  ...
  if (mode === "create") {
    ...
    form.querySelector('[name="service_id"]') &&
